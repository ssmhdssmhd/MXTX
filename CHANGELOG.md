...
### Added
- 🎨 顶部改为渐变横幅（Hero）：服务状态 / 当前版本 / 运行时长三个徽章实时刷新，并提供「万能嗅探 / 嗅探 API / 单页解析 API」导航入口
- 📊 新增 KPI 指标卡：浏览器池、页面复用池（busy/total 与占用率）、嗅探接口数（含熔断数）、LRU 缓存总数（解析 + 嗅探）
- 🖥️ 新增「系统资源」卡片：内存占用 / 浏览器池 / 空闲内存渐变进度条（占用超 60% 变橙、超 85% 变红），并每 15s 采样空闲内存在 Canvas 上绘制近 20 次的迷你走势图
- 🏆 新增「Provider 评分 Top5」榜单：从 `/admin/api/status` 实时拉取评分最高的 5 个第三方解析接口，带奖牌样式、评分条与熔断标记
- ⚙️ 新增「万能嗅探配置」卡片：启用接口数、并发数、提前命中、结果缓存条目、当前熔断接口数
- 🗃️ 新增「缓存详情」卡片：单页解析 / 万能嗅探缓存条目、监听端口、Chrome 状态、更新源
- 🧪 新增「快捷测试」工具：输入视频链接可一键调用「万能嗅探（/sniff）」或「单页解析（/node.js）」，结果格式化内联展示，50s 超时保护
- 📱 响应式适配：小屏下 KPI 与多列网格自动降为单列，便于手机维护

### Changed
- 状态轮询由 30s 收紧到 15s，看板数据与运行时长实时自动刷新

### 验证
- 浏览器实测 `/admin`：控制台无 JS 报错，KPI / 系统资源（含走势图）/ Top5 榜单 / 配置 / 缓存 / 快捷测试 / 在线更新全部正常渲染
- `/admin/api/status` 返回 200 且字段完整，看板取数与在线更新逻辑互不影响
